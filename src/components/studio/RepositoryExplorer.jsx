import { track } from "../../atlas/seo/analytics";
...
const fallback = [
  {
    id: "portfolio",
    name: "react-shubham-21",
    description:
      "The source behind this portfolio. React components, responsive layouts, and interface experiments.",
    language: "JavaScript",
    html_url: "https://github.com/shubham14p3/react-shubham-21",
    homepage: "https://shubhamraj.dev",
...
export default function RepositoryExplorer() {
  ...
  const results = useMemo(
    () => selectRepositories(repositories, { query, language, sort }),
  ...
  return (
    <div className="repository-explorer">
      <div className="repo-toolbar">
        ...
        <label>
          <span className="sr-only">Repository language</span>
          <select
            value={language}
            onChange={(event) => {
      ...
      <p className="repo-status" role="status">
        {status === "loading"
          ? "Refreshing public repositories from GitHub…"
          : status === "error"
            ? "GitHub is unavailable right now. You can still explore this portfolio’s source."
            : `${results.length} public ${results.length === 1 ? "repository" : "repositories"}`}
        {status === "error" && (
          <>
            {" "}
            <button
      ...
      <div className="repository-grid">
        {visible.map((repo) => (
          <article className="repository-card" key={repo.id}>
            ...
            <p>
              {repo.description ||
                `Explore the code and documentation for ${repo.name}.`}
            </p>
            {safeWebUrl(repo.homepage) && (
              <a
                className="text-link"
                href={safeWebUrl(repo.homepage)}
                target="_blank"
                rel="noopener noreferrer"
              >
                Visit project <Icon name="arrow" size={16} />
              </a>
            )}
          </article>
        ))}
